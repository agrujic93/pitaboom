...
const STROKE_INSET = 0;

function buildCardBorderPaths(width, height, r = CARD_RADIUS) {
	const w = width - STROKE_INSET * 2;
	const h = height - STROKE_INSET * 2;
	const o = STROKE_INSET;
...
function initCardBorderDraw(card, index) {
	const svg = card.querySelector('.card-border-draw');
	const rightEl = card.querySelector('.card-border-path--right');
	const leftEl = card.querySelector('.card-border-path--left');

	if (!svg || !rightEl || !leftEl) return;

	// unique gradient ID per card
	const gradientId = `cardBorderGradient-${index}`;
...
document.querySelectorAll('.custom-card-component').forEach((card, index) => {
